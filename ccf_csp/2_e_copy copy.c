...
{    //key 数组用于记录 B 类顶点到 A 类顶点的权值
    ...
    for (int i = 0; i < V; i++) {
        ...
        visited[i] = false_;     // 所有的顶点全部属于 B 类
    ...
    for (int x = 0; x < V - 1; x++)
    {
        ...
        for (int v = 0; v < V; v++)
        {
            ...
            if (cost[u][v] != 0 && visited[v] == false_ && cost[u][v] < key[v])
            {
